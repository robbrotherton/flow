...
grow_line <- function(starting_x, starting_y, data, qtree, ff, steps, step_length, d, line) {
...
make_trail <- function(lines, steps = 10, step_length = .5, d) {
  ...
  line <- 1
  new_line <- grow_line(starting_x, starting_y, out, qtree, ff, steps, step_length, d, line = line)
  # grid <- add_to_grid(grid, new_line$x, new_line$y)
  out <- new_line
  qtree <- update_tree(out)
  queue <- list(new_line)
  seeds <- update_current_line(queue)
  queue <- queue[-1]
  finished <- FALSE
...
update_current_line <- function(queue) {
...
#
t <- make_trail(lines = 10, steps = 200, step_length = 1, d = 1) |>
